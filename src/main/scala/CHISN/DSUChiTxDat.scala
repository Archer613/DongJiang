...
class DSUChiTxDat(snId: Int) (implicit p : Parameters) extends DJModule {
  val nodeParam              = djparam.snNodeMes(snId)
 // -------------------------- IO declaration -----------------------------//
  val io                     = IO(new Bundle {
    val chi                  = Flipped(CHIChannelIO(new CHIBundleDAT(chiParams)))
...
// --------------------- Modules declaration --------------------- //
  val queue                  = Module(new Queue(new CHIBundleDAT(chiParams), entries = nodeParam.nrSnTxLcrdMax, pipe = true, flow = false, hasFlush = false))
...
// ------------------- Reg/Wire declaration ---------------------- //
  ...
  val inFlit                 = WireInit(0.U.asTypeOf(new CHIBundleDAT(chiParams)))
