...
class DSUChiRxRsp(snId: Int) (implicit p : Parameters) extends DJModule {
  val nodeParam                = djparam.snNodeMes(snId)
// -------------------------- IO declaration -----------------------------//
    val io = IO(new Bundle {
      val chi                  = CHIChannelIO(new CHIBundleRSP(chiParams))
      val rxState              = Input(UInt(LinkStates.width.W))
      val flit                 = Flipped(Decoupled(new CHIBundleRSP(chiParams)))
    })
 // -------------------------- Wire/Reg define -----------------------------//
  val lcrdFreeNumReg           = RegInit(0.U((log2Ceil(nodeParam.nrSnRxLcrdMax)+1).W))
  val flitReg                  = RegInit(0.U.asTypeOf(new CHIBundleRSP(chiParams)))
  val flitvReg                 = RegInit(false.B)
  val flit                     = WireInit(0.U.asTypeOf(new CHIBundleRSP(chiParams)))
